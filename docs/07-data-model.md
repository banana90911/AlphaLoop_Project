...
**저장소**: SQLite 파일 1개(`journal.sqlite`). 별도 DB 서버 없음.

**공통 규칙**

- 기본키는 `*_id`(TEXT), 발급 책임은 그 행을 만드는 코드에 있다. 시장 데이터처럼 자연키가 뚜렷한 표는 `(symbolId, tradeDate)` 복합키를 쓴다.
- 시각은 TEXT ISO8601 UTC. 일별 집계용 거래일(`tradeDate`)은 KST 기준 `YYYY-MM-DD`로 따로 둔다.
- 가격·손익·비율은 REAL, 수량·건수는 INTEGER, 참/거짓은 INTEGER(0/1).
- **백테스트 결과는 DB에 넣지 않는다**(6.5). 실거래 표에 `source='backtest'`로 섞지 않으므로, 조건을 빼먹어 백테스트 성과가 실적으로 집계되는 사고가 원천적으로 없다.
...
| 〃      | `ingest_runs`       | 수집 이력·결측 추적        | 후속  |
| 판단     | `cycles`            | 사이클 1회의 상태 머신      | 초기  |
| 〃      | `equity_snapshots`  | 사이클 시점 자본 스냅샷      | 초기  |
| 〃      | `daily_scores`      | 하루 1회 전 종목 점수(1단계) | 초기  |
| 〃      | `cycle_scores`      | 워치리스트 갱신 점수(4단계)   | 초기  |
| 〃      | `decisions`         | 제안 주문 1건           | 초기  |
| 〃      | `risk_checks`       | 게이트 판정 1건          | 초기  |
| 집행     | `orders`            | 주문 송출 1건           | 초기  |
| 〃      | `positions`         | 보유 상태              | 초기  |
| 〃      | `outcomes`          | 청산 실현손익            | 초기  |
| 감사·학습  | `safe_stop_events`  | 전체 정지 발생·해제        | 초기  |
...
> - 4단계에서 보유 종목 손절선을 같은 비율로 조정, 5단계에서 KIS 스톱 주문 정정에 사용
...
| `tradeDate`        | 거래일                                                                                                        |
| `status`           | `intent`→`scoring`(1~3단계)→`deciding`(4~5단계)→`ordering`(6단계)→`recorded`(7단계), 실패 시 `failed`, 건너뜀은 `skipped` |
...
> - 4단계에서 점수를 갱신해 진입·무효 임계를 판정, 5단계 종목 상태 게이트와 6단계 송출 직전 호가 확인에 사용
...
| `trendPercentileLive` | 현재가로 다시 판정한 정배열 백분위                                          |
| `flowPercentileLive`  | 장중 잠정 수급 백분위                                                 |
| `totalScore`          | 갱신 후 종합점수                                                    |
| `lastPrice`           | 사이클 시점 현재가                                                   |
| `buyQuantity`         | 매수 1호가 잔량                                                    |
| `sellQuantity`        | 매도 1호가 잔량                                                    |
| `atr`                 | 손절폭 산출용 ATR                                                  |
| `stopWidth`           | `max(2.5 × ATR, 현재가 × 5%)`                                   |
...
> - 4단계가 낸 제안 주문 1건
> - 4단계에서 생성
> - 5단계 리스크 검증, 6단계 Orders로 송출, 7단계 성과 귀속
...
| `cycleId` (FK)    |                                                                                             |
| `symbolId`        | 종목코드                                                                                        |
| `action`          | `buy`/`exitAll`(전량 청산)/`raiseStop`(본전 상향·트레일링)/`noTrade`                                    |
...
| `winRate`         | 켈리 입력 — 최근 100건 청산의 승률                                                                      |
| `payoffRatio`     | 켈리 입력 — 평균 이익 ÷ 평균 손실                                                                       |
| `riskPercent`     | 자본 대비 위험 비율 = `min(1%, 0.25 × 켈리분수)`                                                        |
| `quantity`        | 제안 수량 = (총자산 × `riskPercent`) ÷ 손절폭, 1주 단위 내림                                               |
| `rewardRiskRatio` | 손익비                                                                                         |
...
> - 5단계 게이트 판정 1건. 검사 7개를 5.2의 고정 순서로 수행한 결과
> - 5단계에서 적재. 여러 규칙이 동시에 걸려도 **가장 먼저 걸린 하나만** 사유로 남긴다
> - 6단계는 `pass`인 결정만 송출한다. 어느 게이트가 자주 걸리는지가 한도 조정의 근거
...
| `reason`          | 단일 사유                                                                                          |
| `limitValue`      | 그때 적용된 한도값(예: 종목당 20%)                                                                         |
...
> - KIS에 보낸 주문
> - 6단계 송출 직후 생성, 체결 확인 시 체결 갱신
> - 5단계 미체결 주문 정리와 상주 스톱의 주문번호 조회, 6단계 ****중복 주문 필터
...
| `symbolId`           | 종목코드                                                                                          |
| `side`               | `buy`/`sell`                                                                                  |
| `purpose`            | `entry`(신규 진입)/`stop`(손절 예약 등록)/`stopAmend`(스톱 정정)/`exit`(능동 매도)                              |
...
| `returnPercent`        | 수익률                                          |
| `rMultiple`            | 손익 ÷ (R × 수량) — 종목·금액이 달라도 비교되는 공통 잣대        |
| `exitReason`           | `stopHit`/`timeExit`/`thesisInvalid`/`trail` |
...
> - 매매 정지 데이터
> - 5단계 safeStop일 때 적재
> - 5단계 사이클 시작 시 미해제 이벤트가 있으면 신규 주문 차단, 감사
...
| `cause`              | 잔고 불일치 / 데이터 오류 / 일일 −4% / API 오류율 급증     |
| `trigger`            | `auto`/`manual`                           |
| `releasedDateTime`   | 해제 시점. **비어 있으면 지금 정지 중**                 |
| `releasedBy`         | 해제자 — 잔고 불일치·데이터 오류는 사람 개입 필수(5.3)        |
| `releaseReason`      | 해제 사유                                     |
