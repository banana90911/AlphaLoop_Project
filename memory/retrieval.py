"""인출 스코어러 — 구조화 매칭 (memory/retrieval, reference/statistics·7.18·7.21·P1-6).

"오늘과 닮은 과거 상황"을 골라 결정에 주입한다. 점수 = 레짐 유사도 + 섹터 + 셋업 +
최근성 + 보정신뢰의 가중 합(임베딩/RAG 미사용 — 자가생성 정형 기억엔 구조화 매칭이 적합).

인출 대상은 **과거 `decisions`↔`outcomes` 원시 매칭**이다(P1-6) — 같은 레짐·섹터·셋업인
과거 결정의 실제 손익을 압축 요약해 주입한다. 빈 결과는 정상(데이터가 쌓이며 작동).
"""
