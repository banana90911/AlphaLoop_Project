...
> - 3단계에서 점수를 갱신해 진입·무효 임계를 판정, 4단계 종목 상태 게이트와 5단계 송출 직전 호가 확인에 사용
...
| `cycleId` (FK)    |                                                                                             |
| `symbolId`        | 종목코드                                                                                        |
| `action`          | `buy`/`exitPartial`(부분 청산)/`exitAll`(전량 청산)/`raiseStop`(트레일링)/`noTrade`                     |
...
| `riskPercent`     | 자본 대비 위험 비율 = `min(2.5%, 0.5 × 켈리분수)`                                                       |
| `quantity`        | 제안 수량. 신규는 (총자산 × `riskPercent`) ÷ 손절폭 1주 단위 내림, `exitPartial`은 보유 × 40%                    |
| `rewardRiskRatio` | 손익비                                                                                         |
...
| `isBreakevenDone`        | +1.5R 부분 청산·본전 상향 완료 여부(중복 실행 방지)                |
...
> - 청산 체결 1건의 실현손익. **부분 청산도 한 행이다**(+1.5R 40% 매도 → 1행, 잔여 청산 → 1행)
> - 6단계 청산이 체결될 때마다 적재
> - 3단계 켈리 승률/손익비
