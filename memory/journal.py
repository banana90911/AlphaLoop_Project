...
# OrderAction(buy/add/hold/trim/sell) → decisions.action·side (07-model schema CHECK 제약).
...
def create_cycle(conn: sqlite3.Connection, cycle_id: str) -> None:
    """`intent` 상태로 사이클 1행 생성(모든 산출물의 부모 키)."""
    conn.execute(
        "INSERT INTO cycles(cycle_id, status, started_at) VALUES(?, ?, ?)",
        (cycle_id, "intent", utc_iso()),
    )
    conn.commit()
    ...
    *,
    ...
    for o in orders:
        ...
        conn.execute(
            ...
            "qty_risk_budget, entry_thesis, stop_loss, source, decided_at) "
            "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (did, cycle_id, o.code, action, side, o.risk_budget, o.thesis or None,
             stops.get(o.code), source, ts),
        )
        ids.append(did)
    conn.commit()
    return ids
