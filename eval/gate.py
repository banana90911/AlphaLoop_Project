"""Go/No-Go 게이트 — 과최적화 검정 + 방향성 게이트 (eval/gate, 11-eval 10-1 P1-5).
...
**Deflated Sharpe(DSR)는 하드 게이트에서 제외(2026-06-21 확정, 11-eval 10-4.3)** — 0.95는
...
def sensitivity_no_cliff(
    ...
    *,
    ...
    """민감도 절벽 검정(11-eval 10-1 ③) — 추천 파라미터를 grid상 한 칸씩 흔든 이웃의 안정성.
...
def dsr_confidence_tier(dsr: float) -> str:
    """DSR(0~1)을 소액 실전 자본 램프업 신뢰도 등급으로 (11-eval 10-1, Phase 7.5/10).
