...
평소 **운영은 KIS 단일 소스**. 네이버 금융은 백테스트 데이터를 **1회 수집**할 때만 쓰는 보조다.

**백테스트 데이터 두 갈래(혼동 주의)**: ① **입력(과거 시세·수급)** = `data/cache/`의 **parquet 캐시**(`data.cache`). 실전 운영 DB와 분리·실전이 읽지 않음 → **실전 전환 시 `cache.clear()`로 폐기 가능**(재수집하면 복원). ② **출력(백테스트 거래결과)** = `journal.sqlite`의 `source='backtest'`(학습 시드). 즉 raw 시세는 실전에 안 넘어가고(①), 거래결과만 학습에 쓰인다(②).
